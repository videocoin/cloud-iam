...
function get_vars() {
    ...
    readonly ENCRYPTION_PASSPHRASE=`consul kv get -http-addr=${CONSUL_ADDR} config/${ENV}/services/${CHART_NAME}/secrets/encryptionPassphrase`
}
...
function deploy() {
    ...
    helm upgrade \
        ...
        --set secrets.encryptionPassphrase="${ENCRYPTION_PASSPHRASE}" \
